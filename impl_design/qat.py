...
from quant_util import quantize_model, ExtractInfo
...
def main(args):    
    ...
    if args.save_onnx:
        ...
        info_path = os.path.join(args.save_dir, f"{args.file_name}_impl_info.json")
        model.transform(ExtractInfo(info_path))
        print(f"[*]\tImplementation report exported in {info_path}")
