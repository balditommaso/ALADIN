...
def print_template_layer(node, layer_type, double_buffering = 2):
    ...
    Args:
        ...
        layer_type: Type of layer (e.g., "Conv2D", "Addition").
    ...
    if not (n_in == tile_n_in and w_in == tile_w_in and h_in == tile_h_in):
        x_buffer_size *= tk["double_buffering"]
    if x_buffer_size % 16 != 0:
        x_buffer_size = x_buffer_size # align if needed
        
    # output buffer
    same_tiling_across_clusters = (
        (n_in == (tile_n_in * number_of_clusters) and w_in == tile_w_in and h_in == tile_h_in and
        n_out == (tile_n_out * number_of_clusters) and n_in > number_of_clusters)
        or
        (n_in == tile_n_in and w_in == tile_w_in and h_in == tile_h_in and
        n_out == (tile_n_out * number_of_clusters))
    )
    if same_tiling_across_clusters:
        # no double buffer
    ...
    # L1 memory offset
    tk['l1_x_offset'] = 0
    tk['l1_y_offset'] = x_buffer_size + 8 # +8 is for safety margin
    ...
    if "Addition" not in node.name and "Pool" not in node.name:
        ...
        if tk['FLAG_BATCHNORM'] == 1:
            ...
            tk['l1_lambda_offset'] = sum((
                x_buffer_size + 8,
                y_buffer_size + 8,
                W_buffer_size + 8, 
                tk['k_tile_size_byte'] + 8
            ))
        if has_bias:
            tk['l1_b_offset'] = sum((
                x_buffer_size + 8,
    ...
    # clip last-tile dimensions (avoid overshoot)
    ...
    if "Addition" not in node.name and "Pool" not in node.name:
        ...
        tk['im2col_dim'] = (
            8 * (fs1 * (tile_h_in + padding_bottom + padding_top) + fs1)
        ) * int( 8 / min(ds_x, ds_y, ds_W))

        tk['lut_dim'] = 0
        if tk["flag_LUT"]:
            num_entries = 2 ** (ds_x + ds_W)
            tk['lut_dim'] = num_entries * 4 + 8
        ...
        buffer_l1_all = sum((
            x_buffer_size, 
            y_buffer_size,
            tk['k_tile_size_byte'], tk['lambda_tile_size_byte'],
            40,
            tk['b_size_byte']
        ))
